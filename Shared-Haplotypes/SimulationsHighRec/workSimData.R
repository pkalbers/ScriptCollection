...
ibd.segments = function(target, shared, M, hap.file, ibd.file, mc.cores = NULL) {
	scan = function(ibd, M, hap.file, ibd.file) {
		...
		ibd$other.hap = if(is.mat) pair[1] else pair[2]
		ibd$other.lab = if(is.mat) "M" else "P"
		...
		sub = if (length(match) != 1) as.matrix(IBD[match, ]) else t(as.matrix(IBD[match, ]))
		...
		if (length(match) > 1) {
			sub = if (length(match) != 1) as.matrix(sub[match, ]) else t(as.matrix(sub[match, ]))
			dst = sub[, "end"] - sub[, "beg"]
			match = which.min(dst)
		} 
		
		ibd$beg = as.numeric(sub[match, "beg"])
		ibd$end = as.numeric(sub[match, "end"])
	...
	if (is.null(mc.cores) || is.na(mc.cores)) {
		for (i in 1:length(ibd)) {
			ibd[[i]] = scan(ibd[[i]], M, hap.file, ibd.file)
		}
	} else {
...
for (target in sample(I, 10)) {
	cat(target, "\n")
	
	shared = sharing[[target]]
	
	esh = esh.segments(target, shared, M, gen.file, mc.cores = cores)
	ibd = ibd.segments(target, shared, M, hap.file, ibd.file, mc.cores = cores)
