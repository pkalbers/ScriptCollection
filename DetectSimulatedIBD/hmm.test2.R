...
prefix = "history" #"history.generror_1000g"
...
emiss = array(NA, c(2, 6, length(POS)), dimnames = list(c("IBD", "NON"), colnames(hmm.emiss.ibd), NULL))
for (i in 1:length(POS)) {
	tmp1 = hmm.emiss.ibd[AC[i] + 1, ] #+ 1e-8
	tmp2 = hmm.emiss.non[AC[i] + 1, ] #+ 1e-8
	
	#tmp1 = tmp1 * c(1, 1, 0.5, 1.1, 1, 1) # tmp1 * exp(-1 * tmp1) # 1 - exp(-1 * tmp1)
	#tmp2 = tmp2 * c(1, 1, 1.5, 0.9, 1, 1) # tmp2 * exp(-1 * tmp2) # 1 - exp(-1 * tmp2)
...
fki = FKI[sample(which(FKI$n.sharer == 4), 1), ]
fki = FKI[which(FKI$index == 168524), ]
...
trans = sapply(1:length(POS), function(x) get.trans.prob(dist.fwd[x], AC[fki$index], sample.size), simplify = "array")
...
for (share in 1:ncol(g.sharers)) {
	...
	d.emis = rbind(d.emis, data.table(tag = tag, idx = 1:length(POS), pos = POS, ibd = e.ibd, non = e.non)) # diff = e.non - e.ibd))
	...
	d.othr = rbind(d.rare, data.table(tag = tag,  
																		idx = rare,  
																		pos = POS[rare],
																		frq = AC[rare]))
...
d.outp1 = d.emis
d.outp1$brk = cut(d.outp1$pos, 500)
...
d.outp2 = d.emis
d.outp2$brk = cut(d.outp2$pos, 1000)
...
d.outp3 = d.emis
d.outp3$brk = cut(d.outp3$pos, 5000)
...
d.outp4 = d.emis
d.outp4$brk = cut(d.outp4$pos, 10000)
...
gg = ggplot(d.post) + 
	...
	geom_linerange(data = d.true, aes(x = pos, ymin = 2.2, ymax = 3.2), colour="white") +
	geom_linerange(data = d.true, aes(x = pos, ymin = 2.2, ymax = 3.2), colour="black", linetype = "22") +
	...
	geom_point(data = d.rare, aes(x = pos, y = 3.65), colour = "black", size = 1) +
	geom_point(data = d.othr, aes(x = pos, y = 3.45), colour = "black", size = 1, shape = 1) +
	#geom_point(data = d.post, aes(x = pos, y = 2.9)) +
	scale_x_continuous(expand = c(0, 0), breaks = seq(0, 100e6, by = 10e6)) +
	...
	#coord_cartesian(xlim = c(max(POS[1], fki$pos - 1e6), min(POS[length(POS)], fki$pos + 25e6))) +
	theme_few() +
...
ggsave(gg, filename = sprintf("_plot.hmm.emissions2.%d.%s.pdf", fki$index, prefix), width = 25, height = nrow(d.true) * 1/2 + 3, limitsize = F)
...
e.non = rep(NA, length(obs))
for (i in 1:length(obs)) {
	tmp = emiss["IBD", , i] # 1 - exp(-1 * emiss["IBD", , i])
	tmp = tmp / sum(tmp)
	e.ibd[i] = tmp[obs[i]]
	
	tmp = emiss["NON", , i] # 1 - exp(-1 * emiss["NON", , i]) 
	tmp = tmp / sum(tmp)
	e.non[i] = tmp[obs[i]]
...
d.emis = rbind(d.emis,
							 data.table(tag = tag, idx = 1:length(POS), pos = POS, diff = e.non - e.ibd))




d.emis$brk = cut(d.emis$idx, 500)
d.emis = split(d.emis, list(d.emis$tag, d.emis$brk))
d.emis = lapply(d.emis, function(x) {
	z = x[1, ]
	z$diff = mean(x$diff)
	z$idx.end = x$idx[nrow(x)]
	z$pos.end = x$pos[nrow(x)]
...
d.emis = rbindlist(d.emis)


ggplot(d.post) + 
	facet_grid(tag~.) +
	geom_hline(yintercept = c(0, 0.5, 1), colour = "grey80") +
	geom_line(aes(x = idx, y = pprob)) +
	geom_rect(data = d.emis, aes(xmin = idx, xmax = idx.end, ymin = -1.2, ymax = -0.1, fill = diff)) +
	geom_linerange(data = d.true, aes(x = idx, ymin = -0.9, ymax = -0.4), colour="white") +
	geom_linerange(data = d.true, aes(x = idx, ymin = -0.9, ymax = -0.4), colour="black", linetype = "22") +
	scale_fill_gradient2(low = "orange", high = "royalblue1", mid = "white", midpoint = 0, limits=c(-0.03, 0.03)) 
