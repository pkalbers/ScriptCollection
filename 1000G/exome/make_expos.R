...
bed = "20141020.strict_mask.whole_genome.bed"
...
files = dir(".+\\.marker\\.txt$", path = "../data", full.names = T)
...
for (file in files) {
	...
	str = sprintf("chr%s", chr)
	cat(chr, "\n")
	
	m = fread(file, header = T, stringsAsFactors = F)
	b = bed[[str]]
	
	tmp = sapply(m$Position, function(x, z) {
...
save(strict.mask, file = "strict.mask.RData")
...
exo = "20130108.exome.targets.bed"
exo = fread(exo, header = F, stringsAsFactors = F)
exo = split(exo, exo$V1)
...
save(exome.mask, file = "exome.mask.RData")
