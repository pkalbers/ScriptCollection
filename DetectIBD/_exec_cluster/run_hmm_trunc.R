...
hmm.viterbi = function (obs, init, emiss, trans, cutoff = NULL) {
	n.obs = length(obs)
	n.sts = length(init)
	states = 1:n.sts
	...
	for (i in 2:n.obs) {
		...
		if (!is.null(cutoff) && v[1, i] < cutoff) {
			n.obs = i
			break
		}
	}
...
detect.shap.hmm = function(focal, G, POS, INIT, EMISS, TRANS, treshold) {
	foc = focal$index
	fk  = focal$fk
	g0  = focal$g0
	g1  = focal$g1
	...
	lhs.path = hmm.viterbi(lhs.obs, init, lhs.emiss, lhs.trans, treshold)
	rhs.path = hmm.viterbi(rhs.obs, init, rhs.emiss, rhs.trans, treshold)
	
	lhs = match(2, lhs.path)
	rhs = match(2, rhs.path)
...
run.hmm = function(pair, G, POS, RATE, Ne, INIT, EMISS, TRANS, log.file, save.file) {
	...
	for (i in 1:nrow(pair)) {
		...
		hmm.lr = detect.shap.hmm(pair[i, ], G, POS, INIT, EMISS, TRANS, 1e-8)
		pair$hmm.t8.lhs[i] = hmm.lr[1]
		pair$hmm.t8.rhs[i] = hmm.lr[2]
		
		hmm.lr = detect.shap.hmm(pair[i, ], G, POS, INIT, EMISS, TRANS, 1e-32)
		pair$hmm.t32.lhs[i] = hmm.lr[1]
		pair$hmm.t32.rhs[i] = hmm.lr[2]
		
		hmm.lr = detect.shap.hmm(pair[i, ], G, POS, INIT, EMISS, TRANS, 1e-128)
		pair$hmm.t128.lhs[i] = hmm.lr[1]
		pair$hmm.t128.rhs[i] = hmm.lr[2]
...
for (pair.file in sample(pair.files)) {
	...
	save.file = sprintf("result_hmm_trunc.%s", pair.file)
	log.file  = sprintf("log_hmm_trunc.%s.txt", pair.file)
	tmp.file  = sprintf("tmp_hmm_trunc.%s", pair.file)
