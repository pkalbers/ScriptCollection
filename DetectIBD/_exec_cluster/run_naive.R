...
if (mode == "G") load(sprintf("data.%s.G.RData", prefix)) # genotypes
if (mode == "H") load(sprintf("data.%s.H.RData", prefix)) # haplotypes
if (mode == "P") load(sprintf("data.%s.P.RData", prefix)) # phased haplotypes
...
for (pair.file in sample(pair.files)) {
	...
	save.file = sprintf("result_%s.%s", mode, pair.file)
	log.file  = sprintf("log_%s.%s.txt", mode, pair.file)
	...
	if (mode == "G") pair = run.gen(pair,  G, POS, log.file)
	if (mode == "H") pair = run.hap(pair,  H, POS, log.file)
	if (mode == "P") pair = run.phap(pair, P, POS, log.file)
