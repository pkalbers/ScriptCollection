...
for (file in dir(pattern = "^res_.+\\.age\\.sites\\..+txt$", path = "./res", full.names = T)) {
...
table(d$phased, d$method, d$clock)


p = d
p = p[which(p$error),]
p = p[which(p$method == "H-HMM"),]
p = p[which(p$tmrca == "100"),]
...
tm = rbind(data.table(x = 10^(0:6), y = 0.0001, xend = 10^(0:6), yend = 1.3),
					 data.table(y = 10^(0:6), x = 0.0001, yend = 10^(0:6), xend = 1.3))


scat = ggplot(p) +
	facet_grid(phased~clock) +
	geom_raster(aes(x, PostMode * Ne), position = "identity", stat = "bin2d", binwidth = c(0.095, 0.095)) +
	geom_tile(aes(x, PostMode * Ne), position = "identity", stat = "bin2d", binwidth = c(0.095, 0.095)) +
	...
	scale_fill_gradientn(colours = c("grey90", "grey80", "lightskyblue", "deepskyblue", "dodgerblue", "purple1", "purple4", "navy", "black"), na.value = "black", trans = "log10", limits = c(1, 100)) +
...
ggsave(scat, filename = sprintf("_plot.scat.hhmm_.t%s.pdf", p$tmrca[1]), height = 5, width = 7.2)
#ggsave(scat, filename = "_plot.scat.hhmmi.pdf", height = 5, width = 7.2)




x = (p$parent.time > p$PostMode * Ne & p$PostMode * Ne > p$node.time)
x = table(p$clock, p$phased, x)
x / rowSums(x) * 100
...
a = sapply(split(p, list(p$clock, p$phased)), function(x) {
	data.table("Child node"  = round(cor(x$PostMode * Ne, x$node.time, method = "s"), 4),
						 "Midpoint"    = round(cor(x$PostMode * Ne, x$mid, method = "s"), 4),
						 "Parent node" = round(cor(x$PostMode * Ne, x$parent.time, method = "s"), 4))
	
})
a = t(array(a, dim = dim(a), dimnames = dimnames(a)))
a

b = sapply(split(p, list(p$clock, p$phased)), function(x) {
	data.table("Child node"  = round(rmsle(x$PostMode * Ne, x$node.time), 4),
						 "Midpoint"    = round(rmsle(x$PostMode * Ne, x$mid), 4),
						 "Parent node" = round(rmsle(x$PostMode * Ne, x$parent.time), 4))
...
x = by(p, list(p$Fk, p$clock, p$phased), function(x) cor(x$PostMode * Ne, x$node.time, method = "s"))
...
q = data.table(fk = p$Fk, clock = p$clock, phased = p$phased, est = p$PostMode * Ne, tru = p$mid, t0 = p$node.time, t1 = p$parent.time)

q$map = log(q$est / q$t0) / log(q$t1 / q$t0)

tmp = q
#tmp = tmp[which(tmp$fk %in% c(0, 2, 5, 10, 15, 20, 25)), ]
tmp = split(tmp, list(tmp$clock, tmp$phased, tmp$fk))
tmp = lapply(tmp, function(z) {
	if (nrow(z) == 0) return(NULL)
	x = seq(-10, 10, by=0.05)
	y = ecdf(z$map)(x)
	data.table(fk = z$fk[1], clock = z$clock[1], phased = z$phased[1], x=x, y=y)
})
q = rbindlist(tmp)
...
hist = ggplot(q) + 
	...
	geom_line(aes(x, y, color = factor(fk)), size = 2/3, alpha = 3/4) +
	...
	scale_color_manual(values = colorRampPalette(rev(c(colorRampPalette(c("grey90", "maroon1"))(3)[2],
																										 ...
																										 colorRampPalette(c("grey95", "darkorange3"))(3)[2])))(length(unique(q$fk)))) +
	...
	theme(aspect.ratio = 1/2,
				...
				legend.position = "bottom",#c(0.005, 0.99), legend.justification = c(c(0,1)), legend.margin = margin(0,1,1,1, "mm"),
				strip.text.x = element_text(face = "bold", hjust = 0, size = 10),
				strip.text.y = element_text(face = "bold", angle = -90, size = 11),
...
ggsave(hist, filename = sprintf("_plot.hist.hhmm_.t%s.pdf", p$tmrca[1]), height = 5, width = 11)
ggsave(hist, filename = "_plot.hist.hhmmi.pdf", height = 5, width = 11)
