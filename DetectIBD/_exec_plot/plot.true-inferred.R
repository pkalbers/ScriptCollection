...
# unique
key1 = sprintf("%.01f %.01f %d %d", match$h.lhs, match$h.rhs, match$h0, match$h1);  length(unique(key1)) / nr * 100
key2 = sprintf("%.01f %.01f %d %d", match$p.lhs, match$p.rhs, match$h0, match$h1);  length(unique(key2)) / nr * 100
key3 = sprintf("%.01f %.01f %d %d", match$g.lhs, match$g.rhs, match$h0, match$h1);  length(unique(key3)) / nr * 100
key4 = sprintf("%.01f %.01f %d %d", match$true.lhs.idx, match$true.rhs.idx, match$h0, match$h1);  length(unique(key4)) / nr * 100

del1 = duplicated(key1)
del2 = duplicated(key2)
del3 = duplicated(key3)
del4 = duplicated(key4)

del = unique(c(which(del1), which(del2), which(del3), which(del4)))
(1 - (length(del) / nr)) * 100
match = match[-del, ]
nrow(match) / nr * 100
nr = nrow(match)
...
match$h.wall = (match$h.lhs <= wl | match$h.rhs >= wr)  ;  t=table(match$h.wall); t/sum(t)*100
match$p.wall = (match$p.lhs <= wl | match$p.rhs >= wr)  ;  t=table(match$p.wall); t/sum(t)*100
match$g.wall = (match$g.lhs <= wl | match$g.rhs >= wr)  ;  t=table(match$g.wall); t/sum(t)*100
match$true.wall = (match$true.lhs.idx == 1 | match$true.rhs.idx == length(POS))  ;  t=table(match$true.wall); t/sum(t)*100
...
d = rbind(data.table(pos = match$pos, act = POS[match$true.lhs.idx], inf = match$g.lhs, side="LHS", fk = match$fk, type = "(c) DGT, genotypes",          det = match$pos - match$g.lhs + 1,   tru = match$pos - POS[match$true.lhs.idx] + 1),
					data.table(pos = match$pos, act = POS[match$true.lhs.idx], inf = match$h.lhs, side="LHS", fk = match$fk, type = "(a) FGT, true haplotypes ",   det = match$pos - match$h.lhs + 1,   tru = match$pos - POS[match$true.lhs.idx] + 1),
					data.table(pos = match$pos, act = POS[match$true.lhs.idx], inf = match$p.lhs, side="LHS", fk = match$fk, type = "(b) FGT, phased haplotypes ", det = match$pos - match$p.lhs + 1,   tru = match$pos - POS[match$true.lhs.idx] + 1),
					#data.table(pos = match$pos, act = POS[match$true.lhs.idx], inf = match$hmm.lhs, side="LHS", fk = match$fk, type = "Hidden Markov Model",         det = match$pos - match$hmm.lhs, tru = match$pos - POS[match$true.lhs.idx]),
					data.table(pos = match$pos, act = POS[match$true.rhs.idx], inf = match$g.rhs, side="RHS", fk = match$fk, type = "(c) DGT, genotypes",          det = match$g.rhs - match$pos + 1,   tru = POS[match$true.rhs.idx] - match$pos + 1),
					data.table(pos = match$pos, act = POS[match$true.rhs.idx], inf = match$h.rhs, side="RHS", fk = match$fk, type = "(a) FGT, true haplotypes ",   det = match$h.rhs - match$pos + 1,   tru = POS[match$true.rhs.idx] - match$pos + 1),
					data.table(pos = match$pos, act = POS[match$true.rhs.idx], inf = match$p.rhs, side="RHS", fk = match$fk, type = "(b) FGT, phased haplotypes ", det = match$p.rhs - match$pos + 1,   tru = POS[match$true.rhs.idx] - match$pos + 1)
					#data.table(pos = match$pos, act = POS[match$true.lhs.idx], inf = match$hmm.rhs, side="RHS", fk = match$fk, type = "Hidden Markov Model",         det = match$hmm.rhs - match$pos, tru = POS[match$true.rhs.idx] - match$pos)
)

mat = split(match, match$fk)
mat = lapply(mat, function(x) x[sample(1:nrow(x), 10000), ] )
mat = rbindlist(mat)

q = rbind(data.table(side="LHS", fk = mat$fk, type = "(c) DGT, genotypes",          det = mat$pos - mat$g.lhs + 1,   tru = mat$pos - POS[mat$true.lhs.idx] + 1),
					data.table(side="LHS", fk = mat$fk, type = "(a) FGT, true haplotypes ",   det = mat$pos - mat$h.lhs + 1,   tru = mat$pos - POS[mat$true.lhs.idx] + 1),
					data.table(side="LHS", fk = mat$fk, type = "(b) FGT, phased haplotypes ", det = mat$pos - mat$p.lhs + 1,   tru = mat$pos - POS[mat$true.lhs.idx] + 1),
					#data.table(side="LHS", fk = mat$fk, type = "Hidden Markov Model",         det = mat$pos - mat$hmm.lhs, tru = mat$pos - POS[mat$true.lhs.idx]),
					data.table(side="RHS", fk = mat$fk, type = "(c) DGT, genotypes",          det = mat$g.rhs - mat$pos + 1,   tru = POS[mat$true.rhs.idx] - mat$pos + 1),
					data.table(side="RHS", fk = mat$fk, type = "(a) FGT, true haplotypes ",   det = mat$h.rhs - mat$pos + 1,   tru = POS[mat$true.rhs.idx] - mat$pos + 1),
					data.table(side="RHS", fk = mat$fk, type = "(b) FGT, phased haplotypes ", det = mat$p.rhs - mat$pos + 1,   tru = POS[mat$true.rhs.idx] - mat$pos + 1)
					#data.table(side="RHS", fk = mat$fk, type = "Hidden Markov Model",         det = mat$hmm.rhs - mat$pos, tru = POS[mat$true.rhs.idx] - mat$pos)
)
...
d = rbind(data.table(side="LHS", fk = match.H$fk, type = "(a) Beagle IBD, true haplotypes",    det = match.H$position - match.H$beagle.lhs.position, tru = match.H$position - POS[match.H$lhs.index]),
					data.table(side="LHS", fk = match.P$fk, type = "(b) Beagle IBD, phased haplotypes",  det = match.P$position - match.P$beagle.lhs.position, tru = match.P$position - POS[match.P$lhs.index]),
					data.table(side="RHS", fk = match.H$fk, type = "(a) Beagle IBD, true haplotypes",    det = match.H$beagle.rhs.position - match.H$position, tru = POS[match.H$rhs.index] - match.H$position),
					data.table(side="RHS", fk = match.P$fk, type = "(b) Beagle IBD, phased haplotypes",  det = match.P$beagle.rhs.position - match.P$position, tru = POS[match.P$rhs.index] - match.P$position))
...
# over/underest
ou = data.table(r = round(match$g.rhs / 10), tr = round(match$true.rhs.pos / 10),
								l = round(match$g.lhs / 10), tl = round(match$true.lhs.pos / 10))
...
ggsave(filename = "___plot.break.hist.pdf", plot = hist, width = 9, height = 9)
...
ggsave(filename = "___plot.break.scat.pdf", plot = scatter, width = 9, height = 9)
### OR:
ggsave(filename = sprintf("_plot.beagle.scatter.%s.pdf", prefix), plot = scatter, width = 9, height = 6)
...
h.phy = ((match$h.rhs - match$h.lhs)+1) / 1e6
p.phy = ((match$p.rhs - match$p.lhs)+1) / 1e6
g.phy = ((match$g.rhs - match$g.lhs)+1) / 1e6
t.phy = ((match$true.rhs.pos - match$true.lhs.pos)+1) / 1e6
...
q = rbind(data.table(idx = match$index, fk = match$fk, type = "(a) FGT, true haplotypes",    x = h.phy, mode = "A. Physical length"),
					data.table(idx = match$index, fk = match$fk, type = "(a) FGT, true haplotypes",    x = h.gen, mode = "B. Genetic length"),
					data.table(idx = match$index, fk = match$fk, type = "(b) FGT, phased haplotypes",  x = p.phy, mode = "A. Physical length"),
					data.table(idx = match$index, fk = match$fk, type = "(b) FGT, phased haplotypes",  x = p.gen, mode = "B. Genetic length"),
					data.table(idx = match$index, fk = match$fk, type = "(c) DGT, genotypes",          x = g.phy, mode = "A. Physical length"),
					data.table(idx = match$index, fk = match$fk, type = "(c) DGT, genotypes",          x = g.gen, mode = "B. Genetic length"),
					data.table(idx = match$index, fk = match$fk, type = "True IBD",                x = t.phy, mode = "A. Physical length"),
					data.table(idx = match$index, fk = match$fk, type = "True IBD",                x = t.gen, mode = "B. Genetic length"))
...
tl = c("True IBD", "(a) FGT, true haplotypes","(b) FGT, phased haplotypes","(c) DGT, genotypes")
