...
key = sprintf("%d %d %d %d", tru$g0, tru$g1, tru$lhs.index, tru$rhs.index)
...
full$indv = sprintf("%d %d", full$g0, full$g1)
uniq$indv = sprintf("%d %d", uniq$g0, uniq$g1)





d = fread("../beagle_ibd.OutOfAfricaHapMap20.H.ibd", header = F, stringsAsFactors = F)
d = fread("../beagle_ibd.OutOfAfricaHapMap20.P.ibd", header = F, stringsAsFactors = F)
...
d$indv = sprintf("%d %d", d$g0, d$g1)


## remove overlapping haplotype segments if PHASED
z = split(d$pair, d$indv)
t = lapply(z, table)
...
y = unlist(x, use.names = F)
z = which(d$pair %in% y)
z = d[z,]
z = z[order(z$indv, z$beg),]
diff = z$beg[-1] - z$end[-(nrow(z))]
w0 = which(diff < 0)
w1 = w0 + 1
w = which(z$indv[w0] == z$indv[w1])
w0 = w0[w]
w1 = w1[w]
zz = data.table(a = z$tag[w0], b = z$tag[w1])
zz = apply(zz, 1, function(x) sample(x, 1))
length(zz)
ww = which(d$tag %in% zz)
...
length(unique(dh$pair))  #  6910821
length(unique(dp$pair))  #  6855710
...
length(intersect(unique(dh$pair), unique(dp$pair)))  #  4377713
length(intersect(unique(dh$indv), unique(dp$indv)))  #  4377713
...
pairs.h = intersect(unique(dh$indv), unique(uniq$indv))  #  1889137
pairs.p = intersect(unique(dp$indv), unique(uniq$indv))  #  1886876
...
mean(p.tru)  #  H : 0.4297985  P : 0.4157889
mean(p.est)  #  H : 0.9782140  P : 0.9739512
...
ggplot(q) +
	facet_grid(.~mode) +
	stat_bin(aes(p, ..density.., color = tag, linetype = type), bins = 21, position = "identity", geom = "line", size=2/3) +
	stat_bin(aes(p, ..density.., color = tag, linetype = type, shape = tag), bins = 21, position = "identity", geom = "point", size=1.5) +
	scale_x_continuous(breaks = seq(0, 1, length.out = 11), labels = seq(0, 100, length.out = 11)) +
	...
	xlab("Overlap (%)") + ylab("Density") 

ggsave(filename = "__plot.beagle_overlaps.err.pdf", width = 8, height = 4)
...
pairs = intersect(unique(d$pair), unique(f$pair))

length(pairs)  #  H : 2332363 , P : 1661487 (1886876)

ddd = d[which(d$pair %in% pairs), ]
ttt = f[which(f$pair %in% pairs), ]
...
ddd$tag = 1:nrow(ddd)  #  H :  6260906  P : 3740239
ttt$tag = 1:nrow(ttt)  #  H : 10927033  P : 7538333
...
length(idx)  #  H : 9173353  P : 2303056 (8958549)
...
range(ttt$g0[xt] - ddd$g0[xd])
range(ttt$g1[xt] - ddd$g1[xd])
...
nrow(sub)  #  H : 2107930  P : 529786 (2083768)
...
save(allH, allP, subH, subP, file = "match.beagle.all_sub_segments.err.RData")
