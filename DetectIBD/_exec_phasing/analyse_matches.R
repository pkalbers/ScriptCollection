...
savename = sprintf("phase.rawdata.%04d.RData", idv)
flagname = sprintf("_flag.phase.rawdata.%04d.txt", idv)
...
d = NULL # data.table(fk = "all", idx = IDX, cov = cov)
...
for (tag in names(cov.cum)) {
	...
	d = rbind(d, data.table(fk = as.numeric(tag), idx = IDX[-del], pos = POS[-del], cov = tmp[-del]))
}
...
chunk.crd = data.table(fk = 0, foc = 0, key = "", beg = rep(0, nrow(m)), end = rep(0, nrow(m)), true.beg = rep(0, nrow(m)), true.end = rep(0, nrow(m)))
chunk.seq = list()
...
save(total.cover, cover, count, coord, chunk, file = "tmp.RData")
