...
d = fread("./simtest.age.sites.SIM.Mut.HardBreaks.txt", header = T, stringsAsFactors = F)
d = cbind(d, times[d$MarkerID + 1, ])
d$clock = "Mutation clock"
d$method = "True IBD"
p = rbind(p, d)

d = fread("./simtest.age.sites.SIM.Rec.HardBreaks.txt", header = T, stringsAsFactors = F)
d = cbind(d, times[d$MarkerID + 1, ])
d$clock = "Recombination clock"
d$method = "True IBD"
p = rbind(p, d)

d = fread("./simtest.age.sites.SIM.MutRec.HardBreaks.txt", header = T, stringsAsFactors = F)
d = cbind(d, times[d$MarkerID + 1, ])
...
d = fread("./simres.age.sites.SIM.Mut.HardBreaks.txt", header = T, stringsAsFactors = F)
d = cbind(d, times[d$MarkerID + 1, ])
d$clock = "Mutation clock"
d$method = "True IBD"
p = rbind(p, d)

d = fread("./simres2.age.sites.SIM.Rec.HardBreaks.txt", header = T, stringsAsFactors = F)
d = cbind(d, times[d$MarkerID + 1, ])
d$clock = "Recombination clock"
d$method = "True IBD"
p = rbind(p, d)

d = fread("./simres2.age.sites.SIM.MutRec.HardBreaks.txt", header = T, stringsAsFactors = F)
d = cbind(d, times[d$MarkerID + 1, ])
d$clock = "Combined clock"
d$method = "True IBD"
p = rbind(p, d)
...
d = fread("./detect.age.sites.FGT.Mut.HardBreaks.txt", header = T, stringsAsFactors = F)
d = cbind(d, times[d$MarkerID + 1, ])
d$clock = "Mutation clock"
d$method = "FGT"
p = rbind(p, d)

d = fread("./detect.age.sites.FGT.Rec.HardBreaks.txt", header = T, stringsAsFactors = F)
d = cbind(d, times[d$MarkerID + 1, ])
d$clock = "Recombination clock"
d$method = "FGT"
p = rbind(p, d)

d = fread("./detect.age.sites.FGT.MutRec.HardBreaks.txt", header = T, stringsAsFactors = F)
d = cbind(d, times[d$MarkerID + 1, ])
d$clock = "Combined clock"
d$method = "FGT"
p = rbind(p, d)
...
d = fread("./detect.age.sites.DGT.Mut.HardBreaks.txt", header = T, stringsAsFactors = F)
d = cbind(d, times[d$MarkerID + 1, ])
d$clock = "Mutation clock"
d$method = "DGT"
p = rbind(p, d)

d = fread("./detect.age.sites.DGT.Rec.HardBreaks.txt", header = T, stringsAsFactors = F)
d = cbind(d, times[d$MarkerID + 1, ])
d$clock = "Recombination clock"
d$method = "DGT"
p = rbind(p, d)

d = fread("./detect.age.sites.DGT.MutRec.HardBreaks.txt", header = T, stringsAsFactors = F)
d = cbind(d, times[d$MarkerID + 1, ])
d$clock = "Combined clock"
d$method = "DGT"
p = rbind(p, d)
...
d = fread("./detect.age.sites.HMM.Mut.HardBreaks.txt", header = T, stringsAsFactors = F)
d = cbind(d, times[d$MarkerID + 1, ])
d$clock = "Mutation clock"
d$method = "HMM"
p = rbind(p, d)

d = fread("./detect.age.sites.HMM.Rec.HardBreaks.txt", header = T, stringsAsFactors = F)
d = cbind(d, times[d$MarkerID + 1, ])
d$clock = "Recombination clock"
d$method = "HMM"
p = rbind(p, d)

d = fread("./detect.age.sites.HMM.MutRec.HardBreaks.txt", header = T, stringsAsFactors = F)
d = cbind(d, times[d$MarkerID + 1, ])
d$clock = "Combined clock"
d$method = "HMM"
p = rbind(p, d)
...
del = which(p$PostMode == 1e-08)  ; if (length(del) > 0) p=p[-del,]
del = which(p$N_Shared != choose(p$leaves, 2)) ; if (length(del) > 0) p = p[-del,]
table(p$method, p$clock)

mid = split(p$MarkerID, list(p$clock, p$method))
mid = Reduce(intersect, mid)
mid = unique(mid)
del = which(! p$MarkerID %in% mid)
if (length(del) > 0) p=p[-del,]
table(p$method, p$clock)


x = p$MarkerID
save(x, file = "_tmp.RData")


p$mid = exp(log(p$node.time) + ((log(p$parent.time) - log(p$node.time)) / 2))
#mid = sqrt(p$node.time * p$parent.time)
...
TR = as.data.frame(p)[which(p$method == "True IBD"),]
p = as.data.table(as.data.frame(p)[which(p$method != "True IBD"),])


#p$method = factor(p$method, levels = c("True IBD", "FGT", "DGT", "HMM"), ordered = T)
p$method = factor(p$method, levels = c("FGT", "DGT", "HMM"), ordered = T)
...
#tr$method = factor(tr$method, levels = c("True IBD", "FGT", "DGT", "HMM"), ordered = T)
...
scat = ggplot(p) +
	...
	geom_raster(aes(x, PostMode * Ne), position = "identity", stat = "bin2d", binwidth = c(8/100, 8/100)) +
	...
	scale_fill_gradientn(colours = c("grey90", "grey80", "lightskyblue", "deepskyblue", "dodgerblue", "purple1", "purple3", "navy", "black"), na.value = "black", trans = "log10", limits = c(1, 500), breaks = c(1, 10, 100)) +
...
ggsave(scat, filename = "_plot.scat.pdf", height = 7, width = 7.2)



a = sapply(split(p, list(p$method, p$clock)), function(x) {
	data.table("Concordant node"  = cor(x$PostMode * Ne, x$node.time, method = "s"),
						 "Midway node" = cor(x$PostMode * Ne, x$mid, method = "s"),
						 "Discordant node" = cor(x$PostMode * Ne, x$parent.time, method = "s"))
	
})
a = t(array(a, dim = dim(a), dimnames = dimnames(a)))
a

b = sapply(split(p, list(p$method, p$clock)), function(x) {
	data.table("Concordant node"  = rmsle(x$PostMode * Ne, x$node.time),
						 "Midway node"  = rmsle(x$PostMode * Ne, x$mid),
						 "Discordant node" = rmsle(x$PostMode * Ne, x$parent.time))
...
a = lapply(split(p, list(p$leaves, p$method, p$clock)), function(x) {
	data.table(fk = x$leaves[1], method = as.character(x$method)[1], clock = as.character(x$clock)[1],
						 c = cor(x$PostMode * Ne, x$node.time, method = "s"),
						 m = cor(x$PostMode * Ne, x$mid, method = "s"),
						 d = cor(x$PostMode * Ne, x$parent.time, method = "s"))
	
})
a = rbindlist(a)

b = lapply(split(p, list(p$leaves, p$method, p$clock)), function(x) {
	data.table(fk = x$leaves[1], method = as.character(x$method)[1], clock = as.character(x$clock)[1],
						 c = rmsle(x$PostMode * Ne, x$node.time),
						 m = rmsle(x$PostMode * Ne, x$mid),
						 d = rmsle(x$PostMode * Ne, x$parent.time))
	
})
b = rbindlist(b)

save(a, b, file = "_tmp.RData")


cc = cut(p$leaves, breaks = c(2, 5, 10, 20), include.lowest = T)

x = by(p, list(p$clock, p$method), function(x) cor(x$PostMode * Ne, x$node.time, method = "s"))
t(array(x, dim = dim(x), dimnames = dimnames(x)))

x = by(p, list(p$clock, p$method), function(x) cor(x$PostMode * Ne, x$parent.time, method = "s"))
t(array(x, dim = dim(x), dimnames = dimnames(x)))

x = by(p, list(cc, p$clock, p$method), function(x) cor(x$PostMode * Ne, x$node.time, method = "s"))
array(x, dim = dim(x), dimnames = dimnames(x))

x = by(p, list(cc, p$clock, p$method), function(x) cor(x$PostMode * Ne, x$parent.time, method = "s"))
array(x, dim = dim(x), dimnames = dimnames(x))

x = by(p, list(cc, p$clock, p$method), function(x) rmsle(x$PostMode * Ne, x$node.time))
array(x, dim = dim(x), dimnames = dimnames(x))
x = by(p, list(cc, p$clock, p$method), function(x) rmsle(x$PostMode * Ne, x$parent.time))
array(x, dim = dim(x), dimnames = dimnames(x))


cc = cut(p$leaves, breaks = c(2, 5, 10, 20), include.lowest = T)
x = by(p, list(cc, p$clock, p$method), nrow)
array(x, dim = dim(x), dimnames = dimnames(x))



x = by(p, list(p$method, p$clock), function(x) {
	n = nrow(x)
	length(which(x$PostMode * Ne >= x$node.time & x$PostMode * Ne <= x$parent.time)) / n * 100
})
array(x, dim = dim(x), dimnames = dimnames(x))

x = by(p, list(p$method, p$clock), function(x) {
	n = nrow(x)
	length(which(x$PostMode * Ne < x$node.time)) / n * 100
})
array(x, dim = dim(x), dimnames = dimnames(x))

x = by(p, list(p$method, p$clock), function(x) {
	n = nrow(x)
	length(which(x$PostMode * Ne > x$parent.time)) / n * 100
})
array(x, dim = dim(x), dimnames = dimnames(x))

cc = cut(p$leaves, breaks = c(2, 5, 10, 15, 20), include.lowest = T)
x = by(p, list(cc, p$clock, p$method), function(x) {
	n = nrow(x)
	length(which(x$PostMode * Ne >= x$node.time & x$PostMode * Ne <= x$parent.time)) / n * 100
})
array(x, dim = dim(x), dimnames = dimnames(x))
...
q = data.table(fk = p$leaves, clock = p$clock, method = p$method, est = p$PostMode * Ne, tru = p$mid, t0 = p$node.time, t1 = p$parent.time)

q$map = log(q$est / q$t0) / log(q$t1 / q$t0)
#map = log(q$est) / log(q$t1)

x = which(q$method == "True IBD")
q$fk[x] = 2

tmp = q
tmp$fr = cut(tmp$fk, breaks = c(2, 5, 10, 15, 20), include.lowest = T)
#tmp = tmp[which(tmp$fk %in% c(0, 2, 5, 10, 15, 20, 25)), ]
tmp = split(tmp, list(tmp$clock, tmp$method, tmp$fr))
tmp = lapply(tmp, function(z) {
	if (nrow(z) == 0) return(NULL)
	x = seq(-10, 10, by=0.05)
	y = ecdf(z$map)(x)
	data.table(fk = z$fr[1], clock = z$clock[1], method = z$method[1], x=x, y=y)
})
q = rbindlist(tmp)
...
hist = ggplot(q) +
	...
	geom_line(aes(x, y, color = fk), size = 2/3) +
	geom_line(data = r, aes(x, y), color = "black", size = 1, alpha = 1/4) +
	#geom_label(data = gm, aes(x=x, y=y, label=sprintf("%.3f", fac)), color = "grey40", size = 3, hjust=1, label.padding = unit(1,"mm")) +
	...
	scale_color_manual(values = colorRampPalette(rev(c(colorRampPalette(c("grey90", "maroon1"))(3)[2],
																										 ...
																										 colorRampPalette(c("grey95", "darkorange3"))(3)[2])))(5)) +
	coord_cartesian(expand = F, xlim = c(-1.25, 2.25), ylim = c(-0.025, 1.025)) +
	theme_few() +
	theme(aspect.ratio = 1/2,
				...
				legend.box.background = element_rect(fill = "white", size = 1/2, colour = "grey60"),
				legend.position = c(0.0065, 0.985), legend.justification = c(c(0,1)),
				legend.margin = margin(-1,1,0.25, 1, "mm"),
				strip.text.x = element_text(face = "bold", hjust = 0, size = 10),
				strip.text.y = element_text(face = "bold", angle = -90, size = 11),
				panel.grid.major.x = element_line(size = 1/3, colour = "grey85"),
				panel.grid.minor.x = element_blank(),
				panel.grid.major.y = element_line(size = 1/3, colour = "grey85"),
				panel.grid.minor.y = element_blank()) +
	xlab("Relative age estimate (log-scale)") + ylab("CDF") +
	guides(color = guide_legend(override.aes = list(size = 3, alpha = 1)))
hist

ggsave(hist, filename = "_plot.hist.pdf", height = 4.3, width = 7.75)
