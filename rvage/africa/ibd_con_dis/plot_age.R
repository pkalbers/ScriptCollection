...
x = which(d$data == "(a) Before error")
x = which(d$data == "(b) After error")
pp = d[x,]

pp$mid = exp(log(pp$node.time + 1e-8) + ((log(pp$parent.time + 1e-8) - log(pp$node.time + 1e-8)) / 2))
pp$x = pp$mid

x = by(pp, list(paste(pp$Clock, pp$Adjusted), pp$phased, pp$near), function(x) cor(x$x, x$PostMode, method = "spearman"))
(array(x, dim = dim(x), dimnames = dimnames(x)))

x = by(pp, list(paste(pp$Clock, pp$Adjusted), pp$phased, pp$near), function(x) rmsle(x$x, x$PostMode * Ne))
(array(x, dim = dim(x), dimnames = dimnames(x)))

x = by(pp, list(paste(pp$Clock, pp$Adjusted), pp$phased, pp$near), function(x) length(which(x$PostMode * Ne >= x$node.time & x$PostMode * Ne <= x$parent.time)) / nrow(x) * 100)
(array(x, dim = dim(x), dimnames = dimnames(x)))
...
p$Clock = factor(p$Clock, levels = c("(a) Mutation clock", "(b) Recombination clock", "(c) Combined clock"), labels = c("Mutation clock", "Recombination clock", "Combined clock"), ordered = T)
...
scat = ggplot(p) +
	...
	geom_text(data = w, aes(x = 50000, y = 4.8, label = rankc.txt), size = 3.5, hjust = 1, parse = T) +
	geom_text(data = w, aes(x = 50000, y = 2.4, label = rmsle.txt), size = 3.5, hjust = 1, parse = F) +
	geom_text(data = w, aes(x = 50000, y = 1.2, label = props.txt), size = 3.5, hjust = 1, parse = F) +
...
ggsave(scat, filename = ("___plot.cle.tru_NN.pdf"), height = 7.5, width = 11)
ggsave(scat, filename = ("___plot.cle.err_NN.pdf"), height = 7.5, width = 11)
