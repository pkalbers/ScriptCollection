...
files = dir(pattern = "^test\\.OutOfAfricaHapMap20\\.GenErr_1000G\\.H\\.age\\.sites\\.(.+)\\.txt$")
files = dir(pattern = "^test\\.OutOfAfricaHapMap20\\.H\\.age\\.sites\\.(.+)\\.txt$")
files = dir(pattern = "^full\\.OutOfAfricaHapMap20\\.P\\.age\\.sites\\.(.+)\\.txt$")
files=files[-(grep("Hard", files))]
