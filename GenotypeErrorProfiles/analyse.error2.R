...
d = empirical.error(sf78, plotting = T)
...
p78 = p
if (any(is.na(p78$frq.alt))) {
	del = which(is.na(p78$frq.alt))
	p78 = p78[-del,]
}
load("profile.affy6.weak.NA12877.RData")
p77 = p
if (any(is.na(p77$frq.alt))) {
	del = which(is.na(p77$frq.alt))
	p77 = p77[-del,]
}
rm(p)
...
s78 = p
if (any(is.na(s78$frq.alt))) {
	del = which(is.na(s78$frq.alt))
	s78 = s78[-del,]
}
load("profile.affy6.strict.NA12877.RData")
s77 = p
if (any(is.na(s77$frq.alt))) {
	del = which(is.na(s77$frq.alt))
	s77 = s77[-del,]
}
rm(p)
...
# p =rbind(p78, p77)
# p=p[-(which(p$true.is.assumed)),]
# table(p$true.gt == p$call.gt)->q
# q/sum(q)*100


prop = function(x) { length(which(x$true.gt == x$call.gt)) / nrow(x) * 100 }
prop.noassumed = function(x) {
	del = which(x$true.is.assumed)
	if (length(del) > 0) {
		x = x[-del,]
	}
	length(which(x$true.gt == x$call.gt)) / nrow(x) * 100
}
...
100 - prop(rbind(p77,p78))
100 - prop(rbind(s77,s78))

100 - prop.noassumed(rbind(p77,p78))
100 - prop.noassumed(rbind(s77,s78))
...
d$true.gt = sprintf("bold(g[%s])", d$true.gt)
d$call.gt = sprintf("bold(tilde(g)[%s])", d$call.gt)

d$less = "a"

x = which(d$true.n < 1000)
if (length(x) > 0) {
	d$less[x] = "b"
}

x = which(d$true.n < 100)
if (length(x) > 0) {
	d$less[x] = "c"
}
...
gg = ggplot(data = d) + 
	...
	geom_point(aes(x = freq, y = prop, color = less), size = 3/4) +
	scale_x_continuous(expand = c(0.075/3, 0), breaks = c(0, 0.25, 0.5, 0.75, 1), labels = c("0", "0.25", "0.5", "0.75", "1")) +
	scale_y_continuous(expand = c(0.1, 0), breaks = c(0, 0.25, 0.5, 0.75, 1), labels = c("0", "", "0.5", "", "1")) +
	scale_color_manual(values = c(a="black", b="royalblue2", c="orangered2")) +
	theme_few() +
...
ggsave(gg, filename = "_plot.generrprop.1000g.a.pdf", width = 9, height = 3)
ggsave(gg, filename = "_plot.generrprop.1000g.b.pdf", width = 9, height = 3)
ggsave(gg, filename = "_plot.generrprop.affy6.pdf", width = 9, height = 3)
ggsave(gg, filename = "_plot.generrprop.omni.pdf", width = 9, height = 3)
...
del = which(is.na(p$frq.alt))
if (length(del) > 0) {
	p = p[-del,]
}


p$err = p$true.gt != p$call.gt

x = table(p$err)
sprintf("%.3f", x/sum(x)*100)
...
for (tag in names(p)) {
	...
	brk = cut(x$pos, breaks = seq(min(x$pos)-0.5e6, max(x$pos)+0.5e6, by=1e6))
	num = table(brk)
	
	prp = sapply(split(x$err, brk), function(y) {
	...
	d = rbind(d, data.table(chr = as.numeric(tag), brk = names(num), num = as.vector(num), prp = prp))
}


beg = as.numeric(sub("^\\(([^,]+),([^,]+)\\]$", "\\1", d$brk))
end = as.numeric(sub("^\\(([^,]+),([^,]+)\\]$", "\\2", d$brk))
...
# cpos = sapply(split(d, d$chr), function(x) max(x$pos))
# for (i in 2:length(cpos)) {
# 	x = which(d$chr == i)
# 	d$cpos[x] = d$pos[x] + cpos[i-1]
# }
...
alt = lapply(split(d, d$chr), function(x) {
	data.table(chr = x$chr[1], beg = min(x$cpos)-2.5, end = max(x$cpos)+2.5, mid = min(x$cpos) + ((max(x$cpos) - min(x$cpos)) / 2))
})
alt = rbindlist(alt)
alt2 = alt[which(1:nrow(alt) %% 2 == 0), ]

alt3 = data.table(chr = d$chr, x = d$cpos, ymin = 1e-8, ymax = 0.005)
alt3 = lapply(split(alt3, alt3$chr), function(x) { z = which(1:nrow(x) %% 10 == 0); x[z, ] } )
alt3 = rbindlist(alt3)

alt4 = data.table(chr = d$chr, x = d$cpos-0.5, ymin = 1e-8, ymax = 0.0075)
alt4 = lapply(split(alt4, alt4$chr), function(x) { z = 1; x[z, ] } )
alt4 = rbindlist(alt4)

alt5 = data.table(chr = d$chr, x = d$cpos, ymin = 1e-8, ymax = 0.006)
alt5 = lapply(split(alt5, alt5$chr), function(x) { z = which(1:nrow(x) %% 50 == 0); x[z, ] } )
alt5 = rbindlist(alt5)
...
gg = ggplot(d) + 
	geom_linerange(data = alt3, aes(x = x, ymin = ymin, ymax = ymax), size = 1/2, color = "grey60") + 
	geom_linerange(data = alt4, aes(x = x, ymin = ymin, ymax = ymax), size = 0.75, color = "grey60") + 
	geom_linerange(data = alt5, aes(x = x, ymin = ymin, ymax = ymax), size = 1/2, color = "grey60") + 
	geom_rect(data = alt2, aes(xmin = beg, xmax = end, ymin = 1e-8, ymax = 100), fill = "black", alpha = 0.1) +
	geom_point(aes(x = cpos, y = prp * 100), size = 0.25) +
	scale_y_log10(breaks = 10^(1:10)/1000) +
	scale_x_continuous(breaks = alt$mid, labels = alt$chr, expand = c(0,0)) + # , minor_breaks = sort(unique(c(alt$beg, alt$end)))) +
	coord_cartesian(ylim = c(0.005, 10)) +
	theme_few() +
	theme(#aspect.ratio = 1/5,
				panel.border=element_rect(fill = NA, colour = "black", size = 0.75),
				axis.ticks.x = element_blank()) +
	xlab("Chromosome") + ylab("Error density (%)")

gg

ggsave(gg, filename = "_plot.poserrdens.weak.pdf", width = 12, height = 2)
ggsave(gg, filename = "_plot.poserrdens.strict.pdf", width = 12, height = 2)
...
zz = function(file) {
	...
	x = table(p$err)
	x = sprintf("%.3f%%", x/sum(x)*100)[2]
	
	del = which(p$true.is.assumed | p$call.is.assumed)
	if (length(del) > 0) {
		p = p[-del, ]
	}
	
	y = table(p$err)
	y = sprintf("%.3f%%", y/sum(y)*100)[2]
...
setwd("./1000G/")
zz("profile.1000g.weak.NA12878.RData")
zz("profile.1000g.strict.NA12878.RData")


setwd("../omni/")
zz("profile.omni.weak.NA12877.RData")
zz("profile.omni.weak.NA12878.RData")

zz("profile.omni.strict.NA12877.RData")
zz("profile.omni.strict.NA12878.RData")


setwd("../affy6/")
zz("profile.affy6.weak.NA12877.RData")
zz("profile.affy6.weak.NA12878.RData")

zz("profile.affy6.strict.NA12877.RData")
zz("profile.affy6.strict.NA12878.RData")
